//
//  ConfiguredDeviceCollectionViewCell.swift
//  AlpacaClient
//
...
class ConfiguredDeviceCollectionViewCell: UICollectionViewCell {
    ...
    func setup(with device: DiscoveryDeviceSelectorViewModel.ConfiguredDevice) {
        nameLabel.attributedText = NSAttributedString(string: device.name, attributes: [
            .foregroundColor: UIColor.black,
            .font: UIFont.systemFont(ofSize: 14, weight: .regular)
        ])
        
        let descriptionText = "\(device.type) #\(device.number)\n\(device.uniqueID)"
